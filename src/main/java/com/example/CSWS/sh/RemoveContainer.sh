...
RemoveContainer(){
   ...
   # Users 디렉토리로 이동
    ...
    # 개인키 존재 확인 후 삭제, 방화벽도 같이 닫음
    cd ~/Keys
    if [ -e ~/Keys/${containerName} ]; then
        rm ~/Keys/${containerName}
        echo "pikachu99" | sudo -S ufw deny $hPort/tcp
        echo "pikachu99" | sudo -S ufw deny $hPort
        echo "pikachu99" | sudo -S ufw delete deny $hPort/tcp
        echo "pikachu99" | sudo -S ufw delete deny $hPort
    else
        echo "9"; exit 1
    fi
}
