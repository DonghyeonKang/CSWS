...
CreateContainer(){
    ...
    # 도커 컨테이너 실행
    docker run -it --privileged=true -d -p $hPort:$cPort --name $userName$userCode --storage-opt size=$storageSize $imageName /sbin/init
...
OpenPort()
{
    ...
    echo "pikachu99" | sudo -S ufw allow ${portNum}/tcp
}

GetKey()
{
    local cPort=$1 # 컨테이너 포트
...
CreateContainer $1 $2 $3 $4 $5 $6 && OpenPort $1 && GetKey $2 $3 $4 && echo "99"